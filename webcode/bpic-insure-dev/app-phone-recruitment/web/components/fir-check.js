define(function (require, exports) {
    var template = require('./fir-check.html');
        ...
        var recheck = {
            init: function (obj) {
               ...
               /* this.dict = '';*/
                ...
                // 不通过
                this.obj.off('tap', '.fircheck-view .bottom button:last-child');
                this.obj.on('tap', '.fircheck-view .bottom button:last-child', { fun: this}, this.subInterResult);

                // 
                this.obj.off('tap', '.arrow');
                this.obj.on('tap', '.arrow', {fun: this}, this.tabInfo);
                if(this.agentCode){
                    this.firstCheckLogin()
                }else{
                    this.getDict(); 
                }
                var _this = this;
               /* var timer = setInterval(function () {
                    ...
                    if (_this.dict && _this.post.length) {
                        ...
                        $('select[name=protocolPosition]').append(temp(renderData));
                        var name = _this.findName(renderData.post);
                        $('select[name=protocolPosition]').prev('input').val(name);
            ...
            subInterResult: function (btn) {
                ...
                if ($(this).data('result') === 'Y') {
                    data = {
                        ...
                        personnelId: _this.personnelId,
                        result: "1",
                        confirmPosition: $('select[name=protocolPosition]').val()
                    };
                } else {
                    if(!$.trim($('#comment').val())){
                        common.alertCreate({
