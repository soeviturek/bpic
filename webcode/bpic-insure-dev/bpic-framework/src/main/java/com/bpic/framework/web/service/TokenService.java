...
import com.bpic.common.utils.session.SessionCoreUtil;
...
import com.bpic.common.utils.ip.AddressUtils;
...
public class TokenService
{
    ...
    {
        // 获取请求携带的令牌
        String token = getEToken(SessionCoreUtil.getHttpRequest());
    ...
    public int getExpireTime(){
        List<SysDictData> dictDataList = dictDataMapper.selectDictDataByType("expireTime");
        int expireTime= 30 ;
        if(!dictDataList.isEmpty()){
            try {
                expireTime=Integer.parseInt(dictDataList.get(0).getDictValue());
            }catch (Exception e){
               return 30;
            }
        }
     ...
     * @return token
     */
    private String getEToken(HttpServletRequest request)
    {
        String token = null;
